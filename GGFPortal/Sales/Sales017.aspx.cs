...
{
    public partial class Sales017 : System.Web.UI.Page
    {
        字串處理 切字串 = new 字串處理();
        ...
        private void DbInit()
        {
            ...
            strsql.AppendFormat(@"SELECT distinct TOP (100)  a.site,a.ord_nbr,b.cus_item_no, doc_item,a.color_id, color_cname, color_ename,req_date, material_flag, material_check_date 
                    ...
                    where a.material_flag=0 {0} and substring( CONVERT(varchar(10),req_date,111),1,7) between '{1}' and '{2}' and b.vendor_id like'{3}'
                    order by req_date"
                    , strwhere
        ...
        protected void MaterialGV_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if(e.CommandName== "到料")
                using (SqlConnection conn1 = new SqlConnection(strConnectString))
                {
                    ...
                    conn1.Open();
                    transaction1 = conn1.BeginTransaction("UpdateOrdSam");
                    ...
                    try
                    {
                        command1.CommandText = string.Format(@"
                                        update ordc_sample
                                            set material_flag = 1 ,material_check_date =getdate()
                                        where 
                                        site = '{0}' and 
                                        ord_nbr = '{1}' and doc_item = '{2}' and color_id= '{3}' and material_flag = 0
                                        ", StrSite, StrOrd, StrOrdSam, StrColor_id);
                        command1.ExecuteNonQuery();
                        transaction1.Commit();
                        DbInit();
                        //MessageLB.Text = "訂單樣衣料到料日上傳完畢";
                    ...
                    catch (Exception ex1)
                    {
                        try
                        {
                            Log.ErrorLog(ex1, "訂單樣衣料到料日上傳失敗 :", "Sales017.aspx");
                        }
                        catch (Exception ex2)
                        {
                            Log.ErrorLog(ex2, "訂單樣衣料到料日上傳失敗 Error:", "Sales017.aspx");
                        }
                        finally
                        {
